...
void PatternRecognition::init() {
  ...
  if (m_debugPlots) {
    ...
    m_conformalEvents3D =
        new TH3F("m_conformalEvents3D", "m_conformalEvents3D", 200, -0.05, 0.05, 200, -0.05, 0.05, 500, 0, 1);
    ...
    m_cellAngleInverseRadiusMC =
        new TH2F("m_cellAngleInverseRadiusMC", "m_cellAngleInverseRadiusMC", 400, 0, 4000, 1500, 0, 0.03);
...
void PatternRecognition::processRunHeader(LCRunHeader* run) { ++m_runNumber; }
...
void PatternRecognition::processEvent(LCEvent* evt) {
  ...
  if (m_debugPlots) {
    TH2F* conformalEventDisplayMC =
        new TH2F("conformalEventDisplayMC", "conformalEventDisplayMC", 2000, -0.04, 0.04, 2000, -0.04, 0.04);
  ...
  TH2F*    conformalEventDisplay =
      new TH2F("conformalEventDisplay", "conformalEventDisplay", 2000, -0.04, 0.04, 2000, -0.04, 0.04);
  canv->cd();
  ...
  for (int caTrack = 0; caTrack < finalCAtracks.size(); caTrack++) {
    ...
    if (trackHits.size() < m_minClustersOnTrack)
      continue;
...
void PatternRecognition::extendSeedCells(std::vector<Cell*>& cells, std::map<KDCluster*, bool> used,
...
bool PatternRecognition::toBeUpdated(std::vector<cellularTrack> cellularTracks) {
  bool update = false;
  for (int iTrack = 0; iTrack < cellularTracks.size(); iTrack++)
    if (cellularTracks[iTrack].back()->getWeight() > 0) {
      update = true;
      break;
    }
...
void PatternRecognition::followPath(std::vector<cellularTrack>& cellularTracks, int trackNumber,
