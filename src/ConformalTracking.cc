...
void ConformalTracking::buildNewTracks(std::vector<KDTrack*>& conformalTracks, std::vector<KDCluster*>& collection,
  ...
  std::cout << "BUILDING new tracks" << std::endl;
...
void ConformalTracking::extendTracks(std::vector<KDTrack*>& conformalTracks, std::vector<KDCluster*>& collection,
  ...
  for (int currentTrack = 0; currentTrack < nTracks; currentTrack++) {
    ...
    for (unsigned int nKDHit = 0; nKDHit < nKDHits; nKDHit++) {
      // Get the kdHit and check if it has already been used (assigned to a track)
      KDCluster* kdhit = collection[nKDHit];
      //std::cout<<"Detector "<<kdhit->getSubdetector()<<", layer "<<kdhit->getLayer()<<", side "<<kdhit->getSide()<<std::endl;
...
double ConformalTracking::checkReal(KDTrack* track, std::map<KDCluster*, MCParticle*> kdParticles,
  ...
  std::cout << "Track should contain " << uniqueHits << " hits, "
  ...
  for (int th = 0; th < trackHits.size(); th++) {
    std::cout << "Hit " << th << " u = " << trackHits[th]->getU() << " v = " << trackHits[th]->getV()
              << " x = " << trackHits[th]->getX() << " y = " << trackHits[th]->getY() << " z = " << trackHits[th]->getZ()
              << std::endl;
    ...
    std::cout << "Come from particle with id " << particle->getPDG() << " produced at vertexR = " << mcVertexR;

    if (particle->getParents().size() != 0) {
      std::cout << ". Comes from particle with id " << particle->getParents()[0]->getPDG();
    }
    std::cout << std::endl;
  }
  ...
  std::cout << "== Terms in conformal fit: " << track->m_intercept << ", " << track->m_gradient << ", " << track->m_quadratic
            << std::endl;
...
void ConformalTracking::checkReconstructionFailure(MCParticle* particle,
  ...
  // Sort the hits from larger to smaller radius
  std::sort(trackHits.begin(), trackHits.end(), sort_by_radiusKD);
  std::cout << "Track contains " << getUniqueHits(trackHits) << " unique hits. Track size is " << trackHits.size()
            << std::endl;

  for (int th = 0; th < trackHits.size(); th++)
    std::cout << "Hit " << th << " u = " << trackHits[th]->getU() << " v = " << trackHits[th]->getV()
              << " x = " << trackHits[th]->getX() << " y = " << trackHits[th]->getY() << " z = " << trackHits[th]->getZ()
              << ". " << (trackHits[th]->used() ? "Used!" : "") << std::endl;
  ...
  if (std::find(results.begin(), results.end(), trackHits[1]) == results.end()) {
    ...
    std::cout << "- distance between hits is " << sqrt(deltaU * deltaU + deltaV * deltaV) << " and search distance is "
              << m_maxDistance << std::endl;
    std::cout << "- theta of hits is " << trackHits[0]->getTheta() << " and " << trackHits[1]->getTheta()
  ...
  while (hitNumber < (trackHits.size() - 1)) {
    ...
    if (cells.back()->getAngle(cell) > m_maxCellAngle || cells.back()->getAngleRZ(cell) > m_maxCellAngleRZ) {
      if (cells.back()->getAngle(cell) > m_maxCellAngle)
        std::cout << "- cell " << hitNumber << " killed by angular cut. Angle is " << cells.back()->getAngle(cell)
                  << std::endl;
      if (cells.back()->getAngleRZ(cell) > m_maxCellAngleRZ)
        std::cout << "- cell " << hitNumber << " killed by RZ angular cut. Angle is " << cells.back()->getAngleRZ(cell)
                  << std::endl;
      //      for(int c=0;c<cells.size();c++) delete cells[c];
      //      return;
    }
  ...
  if (cellularTracks.size() != 1) {
    std::cout << "- cellular track not produced from cells. Returned " << cellularTracks.size() << " candidates"
              << std::endl;
  ...
  if (finalTracks.size() != 1) {
    std::cout << "- kd track not produced from cellular track. Returned " << finalTracks.size() << " candidates"
              << std::endl;
    for (int c = 0; c < cells.size(); c++)
      delete cells[c];
  ...
  std::cout << "== Terms in conformal fit: " << mcTrack->m_intercept << ", " << mcTrack->m_gradient << ", "
  ...
  if (mcTrack->chi2ndof() > m_chi2cut || mcTrack->chi2ndofZS() > m_chi2cut) {
    if (mcTrack->chi2ndof() > m_chi2cut)
      std::cout << "- track killed by chi2/ndof cut. Track chi2/ndof is " << mcTrack->chi2ndof() << std::endl;
    if (mcTrack->chi2ndofZS() > m_chi2cut)
      std::cout << "- track killed by ZS chi2/ndof cut. Track chi2/ndof in ZS is " << mcTrack->chi2ndofZS() << std::endl;
    for (int c = 0; c < cells.size(); c++)
      delete cells[c];
    for (int t = 0; t < finalTracks.size(); t++)
      delete finalTracks[t];
