...
//      if(depth < (m_minClustersOnTrack-1)) continue; // TODO: check if this is correct
  ...
  for(unsigned int caTrack=0;caTrack<finalCAtracks.size();caTrack++){
    ...
    for(unsigned int trackCell=0;trackCell<finalCAtracks[caTrack].size();trackCell++){
      ...
      // Debug info on hit added
      streamlog_out( DEBUG5 )<<"  -- cluster "<<trackCell+1<<": "<<kdStart->getU()<<","<<kdStart->getV()<<" from detector "<<kdStart->getSubdetector()<<std::endl;
    ...
    // Try to fit
    int fitError = MarlinTrk::createFinalisedLCIOTrack(marlinTrack, trackfitHits, track, MarlinTrk::IMarlinTrack::forward, covMatrix, m_magneticField, m_maxChi2perHit);
  ...
  while(cells.size() != nCells){
    ...
    for(unsigned int itCell=startPos;itCell<nCells;itCell++){
      ...
      for(unsigned int neighbour=0;neighbour<results.size();neighbour++){
        ...
        if(used.count(nhit)) continue;
        if(hit->sameLayer(nhit)) continue;
        if(nhit->getR() >= hit->getR()) continue;
        if(hit->getZ() > 0 && nhit->getZ() < hit->getZ()) continue;
        if(hit->getZ() < 0 && nhit->getZ() > hit->getZ()) continue;
