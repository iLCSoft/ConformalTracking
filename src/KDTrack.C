...
double KDTrack::calculateChi2(){
    ...
    for(int hit=0;hit<m_nPoints;hit++){
        ...
        if(m_rotated){
            ...
            double newdx = dv;
            double newdv = dx;
            xMeasured=newxMeasured; yMeasured=newyMeasured; dx=newdx; dv=newdv;
        }
...
void KDTrack::linearRegression(){
    ...
    // Get the determinant
    double detx = matx[0][0]*(matx[1][1]*matx[2][2] - matx[2][1]*matx[2][1]) - matx[1][0]*(matx[1][0]*matx[2][2] - matx[2][1]*matx[2][0]) + matx[2][0]*(matx[1][0]*matx[2][1] - matx[1][1]*matx[2][0]) ;
